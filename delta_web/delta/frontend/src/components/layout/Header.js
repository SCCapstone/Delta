...
const Header = (props) => {
  ...
  const getNotificationMessages = () =>{
    ...
    .then((res)=>{
      setArrNotificationMessages(res.data)
    })
  }

  // call on load
  useEffect(()=>{
    // only get messages if have token
    if(props.auth.token){
      getNotificationReviews();
  ...
  const authLinks = (
    ...
    */
    ...
    <ul className="navbar-nav me-auto d-flex flex-row mt-3 mt-lg-0">
      ...
      <li className="nav-item text-center mx-2 mx-lg-1">
        <span className="nav-link">
          <Link to="/data/download" style={{textDecoration:"none"}}>
            <div>
      ...
      <li className="nav-item text-center mx-2 mx-lg-1">
        <span className="nav-link">
          <Link to="/data/upload" style={{textDecoration:"none"}}>
            <div>
      ...
      <li className="me-auto nav-item dropdown text-center">
          <span className="nav-link">
            <Link to="/community/organizations" style={{textDecoration:"none"}}>
              <div> 
      ...
      <li className="nav-item text-center mx-2 mx-lg-1">
          <Link className="nav-link" to='/notifications/messages'>
            <div>
              <span className="badge rounded-pill badge-notification bg-success">{arrNotificationMessages.length}</span>
      ...
      <li className="nav-item text-center mx-2 mx-lg-1">
        <Link className="nav-link" to="/notifications/reviews">
          <div>
            <span className="badge rounded-pill badge-notification bg-primary">{arrNotificationReviews.length}</span>
